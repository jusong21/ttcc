...
class NanoProcessor(processor.ProcessorABC):
	...
	def process_shift(self, events, shift_name):
		...
		if self.isArray:
			...
			#pruned_ev['run'] = events.run[req_event]
			pruned_ev['run'] = np.ones(len(channel[req_event]), dtype=int)
			#pruned_ev['luminosityBlock'] = events.luminosityBlock[req_event]
			...
			pruned_ev['ncJet'] = ak.to_numpy(ncjets[req_event])
			...
			out_branch = ["events", "run", "luminosityBlock", "Channel", "trig_bit", "nbJet", "ncJet"]
			if not isRealData:
